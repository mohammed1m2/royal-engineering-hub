        ...
        {divisionsData.map((d) => (
          <Link
            ...
            className={`p-6 rounded-xl border transition-all hover:shadow-md ${
              d.featured ? "border-accent bg-accent/5" : "border-border hover:border-primary"
            }`}
          >
    ...
    <>
      <section className="bg-royal py-20">
        <div className="container max-w-3xl">
          ...
          <division.icon size={40} className="text-gold mb-4" strokeWidth={1.5} />
          <h1 className="text-4xl text-primary-foreground mb-4">{division.name}</h1>
          <p className="text-primary-foreground/60 leading-relaxed">{division.desc}</p>
        </div>
      </section>
      <section className="py-16">
        <div className="container max-w-3xl space-y-12">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {division.technologies.map((t) => (
                  <span key={t} className="border border-border text-sm px-4 py-1.5 rounded-full">{t}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <Link to="/contact" className="inline-flex bg-royal text-primary-foreground font-semibold text-sm px-8 py-3 rounded-[9px] hover:opacity-90 transition-opacity">
